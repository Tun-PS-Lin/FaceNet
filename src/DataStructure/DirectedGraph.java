...
public class DirectedGraph<T> implements GraphInterface<T> {
	...
	public void displayAdjacentVertices(T begin) {
		...
		/**
		 * This method store all the friends in an AList
		 * @param begin - the value of vertex of interest
		 */
		
	//return AList called friendList
	//Go through AList, find the latest post from each friend
	//Display friends posts (3 at a time)
	
		public AList<T> storeAdjacentVertices(T begin) {
			AList<T> friendList = new AList<T>();
	        ...
	        while (vertexIterator.hasNext()) {
	            ...
	            if (hasEdge(begin,vertexNext)) {
	            	//System.out.print(vertexNext + ", ");
	            	friendList.add(vertexNext); //store the ID of friends
	            }
	        }
	        return friendList;
	    }
