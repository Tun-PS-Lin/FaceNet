...
import DataStructure.VertexInterface;
...
public class Main_Program2 {
	...
	// DATABASE FOR POSTS
	private LinkedDictionary<String, Post> ID_post = new LinkedDictionary<String, Post>();
	...
	public void mainProgram() throws IOException {
		...
		// File infilePost = new File("postList.txt");
		//// inputPost = new Scanner(infilePost);
		// populatePost(inputPost);

		populatePost();

		input = new Scanner(System.in);
		// inputPost = new Scanner(System.in);
		...
		writePostToFile();// Update postList.txt before closing
	...
	//
	/**
	 * mainMenu will take in user selection and validate it before return to the
	 * program
	 * 
	 * @param input
	...
	public void registerProfile(Scanner input) {
		...
		AList<String> postList = new AList<String>();
		AList<String> timeStampList = new AList<String>();
		...
		// Add post & ID to dictionary
		Post post = new Post(postList, timeStampList);
		ID_post.add(ID, post);
	...
	/**
	 * logInMenu will prompt user for ID and pass input Program will validate the
	 * input by compare it to the dictionary data Program will only allows for 3
	 * log-in try. More than 3 login attempts (any combination) will return user to
	 * the WELCOME PANEL
	 * 
	 * @param input
	...
	/**
	 * This method displays the Profile Main Selection List, prompt user for a
	 * selection and validate the choice
	 * 
	 * @param input
	...
	/**
	 * This method take the validation choice from profileMainDisplay method and
	 * perform the correspond action
	 * 
	 * @param input
	 *            - Scanner for user input
	 * @param ID
	 *            - String - user ID
	 * @param profile
	 *            - Profile - the profile linked with userID
	 */
	public void profileMainMenu(Scanner input, String ID, Profile profile) {
		...
		while (choice != 6) {
			if (choice == 1) {
				viewPost(input, ID); // wrong txt file
			} else if (choice == 2) {
				System.out.println("\n\n HI " + ID + "!");
				System.out
						.println("-----------------------------------------------------------------------------------");
	...
	/**
	 * This methods will find go through user FriendList, find userID from each of
	 * the friend, remove the user from the friend's FriendList before remove the
	 * user account from the database
	 * 
	 * @param ID
	 *            - String - user ID
	 * @param profile
	 *            - Profile tied with the userID
	 */
	...
	/**
	 * This method display the UPDATE PROFILE MENU and prompt user for input. The
	 * method will validate the user selection before performing the appropriate
	 * action
	 * 
	 * @param input
	 *            - Scanner for user input
	 ...
	 *            - Scanner- user input
	 * @param folderPath
	 *            - String - path to access to Profile picture folder
	...
	/**
	 * This method displays the VIEW FRIEND MENU, prompts user for a selection,
	 * validates it and returns the selection to the program
	 * 
	 * @param input
	 *            - Scanner for user input
	...
	/**
	 * This method takes the valid choice from viewFriendMenuDisplay and performs
	...
	/**
	 * This method performs the search friend action by prompting user for another
	 * account ID The methods will search the database to see if the accountID
	 * exist. If the accountID exits, the methods will search if the accountID is in
	 * the user friendList. If yes, the method will display the information
	 * associated with the accountID If no, the method will display the profile
	 * associated with the accountID and ask user if user want to add friend and
	 * performs accordingly If user searches for their own account, the method will
	...
	/**
	 * This method prompts user selection to add friend. If yes, the method will add
	 * the accountID's profile to user's FriendList and vice versa.
	 * 
	 * @param input
	 *            - Scanner for use input
	 * @param fList
	 *            - AList <String> - user's friendList
	 * @param accountID
	 *            - String - account to be added
	 * @param ID
	 *            -String - userID
	 */
	...
	public void profileAddFriend(Scanner input, String accountID, String ID) {
		...
		if (choice.equals("A")) {
			...
			System.out.println("\nFriend is successfully added! \n" + ID.toUpperCase() + " and "
					+ accountID.toUpperCase() + " are now friends.");
		}
	}

	/**
	 * 
	 * This method will display the friend of a friend from user list and states how
	 * many common friends does the user and the suggested friend has
	 * 
	 * @param ID
	 *            - user ID
	 */
	public void displaySuggestedFriends(String ID) {
		...
		while (!sugggestedFriends.isEmpty()) {
			...
			System.out.printf("%-15s %-29s %-10s \n", friendID, ID_profile.getValue(friendID).getName(),
					mutualFriendsCount);
		}
	}

	/**
	 * This method performs the search friend to be removed by prompting user for
	 * another account ID The methods will search if the accountID is in the user
	 * friendList and prompt user for another try if friend account is not found. If
	 * found, the program will delete the friend account from your list as well as
	 * delete your account from their friendList.
	 * 
	 * @param input
	 *            - Scanner for the user input
	 * @param ID
	...
	/**
	 * Menu display for viewPost
	 * 
	 * @param input
	 * @return
	 */
	public String viewPostMenuDisplay(Scanner inputPost) {
		String choice;
		System.out.println("\n-----------------------------------------------------------------------------------");
		System.out.println("VIEW POST MENU");
		System.out.println("-----------------------------------------------------------------------------------");

		System.out.println("What would you like to do?");
		System.out
				.println("A. Display Friends Posts	\nB. Upload Post\nC. View My Posts \nD. Delete My Posts \nX. Exit");
		System.out.print("\nYour choice: ");

		choice = inputPost.nextLine();
		while (!choice.equalsIgnoreCase("A") && !choice.equalsIgnoreCase("B") && !choice.equalsIgnoreCase("C")
				&& !choice.equalsIgnoreCase("D") && !choice.equalsIgnoreCase("X")) {
			System.out.print("\nInvalid input!\nPlease choose again: ");
			choice = inputPost.nextLine();
		}
		return choice.toUpperCase();
	...
	/**
	 * ask user for a choice within the viewPost Menu
	 * 
	 * @param input
	 * @param ID
	 */
	public void viewPost(Scanner inputPost, String ID) {
		String choice = viewPostMenuDisplay(inputPost);
		while (!choice.equals("X")) {
			if (choice.equals("A")) {
				displayFriendPost(inputPost, ID);
			} else if (choice.equals("B")) {
				uploadPost(inputPost, ID);
			} else if (choice.equals("C")) {
				viewMyPost(inputPost, ID);
			}
			System.out.println("\n");
			choice = viewPostMenuDisplay(inputPost);
		}
	}
	 ...
	 * @param ID
	 */
	public void displayFriendPost(Scanner inputPost, String ID) {
		AList<String> friendList = friendGraph.storeAdjacentVertices(ID); // store the ID of friends in AList
		AList<String> timeStampList = new AList<String>();
		AList<String> IDList = new AList<String>();
		// DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");
		// LocalDateTime now = LocalDateTime.now();
		int postCount = 0;
		// int count;

		System.out.println("\n\nFRIENDS POSTS WITHIN 24 HOURS:\n");

		// Add name + post into Lists
		for (int i = 1; i <= friendList.getLength(); i++) {
			postCount = ID_post.getValue(friendList.getEntry(i)).getPostList().getLength();

			if (postCount > 0) { // if friend has post
				for (int j = 1; j <= postCount; j++) {
					timeStampList.add(ID_post.getValue(friendList.getEntry(i)).getTimeStampList().getEntry(j));
					IDList.add(friendList.getEntry(i));
				}
			}
		}

		sortDescendingChronological(timeStampList, IDList); // Sort from newest to oldest post
		...
		System.out.println("\n\nPUBLIC POSTS:\n");
		length = timeStampList.getLength();
		postCount1 = length;

		while (!choice.equalsIgnoreCase("X") && count1 <= length) {
			if (choice.equalsIgnoreCase("C")) {
				friendName = ID_profile.getValue(IDList.getEntry(count1)).getName();
				postPosition = ID_post.getValue(IDList.getEntry(count1)).getTimeStampList()
						.getPosition(timeStampList.getEntry(count1));

				System.out.print("     " + count1 + ". " + "<" + friendName + ">: \n");
				System.out.print("              "
						+ ID_post.getValue(IDList.getEntry(count1)).getPostList().getEntry(postPosition - 1));
				System.out.println("  (" + timeStampList.getEntry(count1) + ")  \n");
			}

			if (length > 10 && postCount1 > (length % 10)) {
				if (count1 % 10 == 0) {
					System.out.println("                       ...                           \n");
					System.out.println(
							"-----------------------------------------------------------------------------------");
					System.out.println("What would you like to do?");
					System.out.println("A. Like Post \nB. Comment Post \nC. View More Post \nX. Exit");
					System.out.print("\nYour choice: ");

					choice = inputPost.nextLine();
					while (!choice.equalsIgnoreCase("A") && !choice.equalsIgnoreCase("B")
							&& !choice.equalsIgnoreCase("C") && !choice.equalsIgnoreCase("X")) {
						System.out.print("\nInvalid input!\nPlease choose again: ");
						choice = inputPost.nextLine();
					}
					System.out.println();
					if (choice.equalsIgnoreCase("A")) {
						likePublicPost(inputPost, ID);
					} else if (choice.equalsIgnoreCase("B")) {
						commentPublicPost(inputPost, ID);
					} else if (choice.equalsIgnoreCase("C")) {
						System.out.println("\nMORE POSTS: \n");
					}
				}
	 ...
	 * @param IDList
	 */
	public void sortDescendingChronological(AList<String> timeStampList, AList<String> IDList) {
		// Bubble sort timeStampList + IDList
		int compareDate;
		String tempTimeStamp;
		String tempID;

		for (int k = 1; k <= timeStampList.getLength(); k++) {
			for (int l = 2; l <= timeStampList.getLength() - 1; l++) {
				compareDate = timeStampList.getEntry(l).compareTo(timeStampList.getEntry(l - 1));
				if (compareDate < 0) {
					tempTimeStamp = timeStampList.getEntry(l - 1);
					tempID = IDList.getEntry(l - 1);
					timeStampList.replace(l - 1, timeStampList.getEntry(l));
					timeStampList.replace(l, tempTimeStamp);
					IDList.replace(l - 1, IDList.getEntry(l));
					IDList.replace(l, tempID);
				}
			}
		}
		timeStampList.reverse(); // Sort in descending chronological order
		IDList.reverse();
	}

	/**
	 * Asks users to like a post
	 * 
	 * @param inputPost
	 */
	public void likePublicPost(Scanner inputPost, String ID) {
		int postNum;

		System.out.print("\nEnter post number to like: ");
		postNum = Helper.readInt(inputPost);
		while (postNum < 1) {
			System.out.print("\nInvalid input!\nPlease choose again: ");
			postNum = Helper.readInt(inputPost);
		}
		inputPost.nextLine();
		System.out.println("\nYou have successfully liked the post...");
	}

	/**
	 * Asks user to comment on a post
	 * 
	 * @param inputPost
	 */
	public void commentPublicPost(Scanner inputPost, String ID) {
		int postNum;
		String comment;
		...
		inputPost.nextLine();
		System.out.println("\nWrite your comment: ");
		comment = inputPost.nextLine();

		System.out.println("\nYou have successfully commented on the post...");

	}

	/**
	 * Asks user to upload a post
	 * 
	 * @param inputPost
	 * @param ID
	 */
	public void uploadPost(Scanner inputPost, String ID) {
		String userPost = "";
		String userTimeStamp = "";
		AList<String> postList;
		AList<String> timeStampList;
		DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");
		LocalDateTime now = LocalDateTime.now();

		System.out.println("\nWhat's new today? Share with your friends: \n");
		userPost = inputPost.nextLine();
		userTimeStamp = dtf.format(now);

		// Add new post
		postList = ID_post.getValue(ID).getPostList();
		postList.add(userPost);
		ID_post.getValue(ID).setPostList(postList); // update the ID_post dictionary

		// Add new timeStamp of post
		timeStampList = ID_post.getValue(ID).getTimeStampList();
		timeStampList.add(userTimeStamp);
		ID_post.getValue(ID).setTimeStampList(timeStampList); // update the ID_post dictionary

		System.out.println("\nSuccessfully posted! (" + dtf.format(now) + ")\n");
	}

	/**
	 * View the user own post that is within 24 hours at a time
	 * 
	 * @param inputPost
	 * @param ID
	 */
	public void viewMyPost(Scanner inputPost, String ID) {
		int length = 0;
		String choice = "C";
		int count = 1;
		int postCount = 0;
		...
		System.out.println("\n\nMY POSTS:\n");
		length = ID_post.getValue(ID).getPostList().getLength();
		postCount = length;

		while (!choice.equalsIgnoreCase("X") && count <= length) {
			if (choice.equalsIgnoreCase("C")) {
				System.out.print("     " + count + ". " + ID_post.getValue(ID).getPostList().getEntry(postCount));
				System.out.println("  (" + ID_post.getValue(ID).getTimeStampList().getEntry(postCount) + ")  \n");
			}

			if (length > 5 && postCount > (length % 5)) {
				if (count % 5 == 0) {
					System.out.println("                       ...                           \n");
					System.out.println(
							"-----------------------------------------------------------------------------------");
					System.out.println("What would you like to do?");
					System.out.println("A. Edit My Post \nB. Delete My Post \nC. View More Post \nX. Exit");
					System.out.print("\nYour choice: ");
					...
					System.out.println();
					if (choice.equalsIgnoreCase("A")) {
						editMyPost(inputPost, ID);
					} else if (choice.equalsIgnoreCase("B")) {
						deleteMyPost(inputPost, ID);
					} else if (choice.equalsIgnoreCase("C")) {
						System.out.println("\nMORE POSTS: \n");
					}
				}
			} else {
				if (postCount == 1 && choice.equalsIgnoreCase("C")) {
					System.out.println(
							"-----------------------------------------------------------------------------------");
					System.out.println("What would you like to do?");
					System.out.println("A. Edit My Post \nB. Delete My Post	\nX. Exit");
	...
	/**
	 * Menu display for viewMyPost function
	 */
	public void viewMyPostMenuDisplay() {

	}

	/**
	 * Delete the user own post
	 * 
	 * @param inputPost
	 */
	public void deleteMyPost(Scanner inputPost, String ID) {
		int postNum;
		String confrimDelete;
		...
		if (postNum < ID_post.getValue(ID).getPostList().getLength()) {
			System.out.println("\nDo you want to delete the post? (Y/N)");
			System.out.print("Enter choice: ");
			confrimDelete = inputPost.nextLine();
			while (!confrimDelete.equalsIgnoreCase("Y") && !confrimDelete.equalsIgnoreCase("N")) {
				System.out.print("\nInvalid input!\nPlease choose again: ");
				confrimDelete = inputPost.nextLine();
			}
			confrimDelete = confrimDelete.toUpperCase();
			if (confrimDelete.equals("Y")) {
				ID_post.getValue(ID).getPostList().remove(ID_post.getValue(ID).getPostList().getLength() - (postNum-1));
				System.out.println("\nPost successfully deleted.\n");
				return;
			}
			System.out.println("\n");
		} else {
			System.out.println("The post number does not exist...\n");
		}
	}

	/**
	 * Edit the user own post
	 * 
	 * @param inputPost
	 */
	public void editMyPost(Scanner inputPost, String ID) {
		int postNum;
	...
	/**
	 * populate the data from postList.txt to dictionary
	 * 
	 * @param input
	 * @throws IOException
	 */
	public void populatePost() throws IOException {
		{
			String ID = null, userPost = null, timeStamp = null;
			Post post;
			Scanner inputPost;
			...
			inputPost.useDelimiter(":#:"); // Use this delimiter to allow user to have space in the input
			while (inputPost.hasNext()) {
				AList<String> postList = new AList<String>();
				AList<String> timeStampList = new AList<String>();

				ID = inputPost.next();
				userPost = inputPost.next();
				timeStamp = inputPost.next();

				// timeStamp = input.next();
				while (!userPost.equals("/$%^&/")) {
					postList.add(userPost);
					timeStampList.add(timeStamp);
					userPost = inputPost.next();
					timeStamp = inputPost.next();
				}
				post = new Post(postList, timeStampList);
				ID_post.add(ID, post); // Add post and ID to the dictionary database
				inputPost.nextLine();
			}
			inputPost.close();
		}
	...
	/**
	 * write the user ID & post to masterPosts.txt
	 */
	public void writePostToFile() throws IOException {
		String ID;
		Post post;

		try {
			FileWriter writePost = new FileWriter("postList.txt");
			PrintWriter out = new PrintWriter(writePost);
			Iterator<String> keyIterator = ID_post.getKeyIterator();
			Iterator<Post> valueIterator = ID_post.getValueIterator();

			while (keyIterator.hasNext() && valueIterator.hasNext()) {
				ID = keyIterator.next();
				post = valueIterator.next();

				out.print(ID + ":#:");
				// Loop through Post and TimeStamp list, then write to file
				for (int i = 1; i <= post.getPostList().getLength(); i++) {
					out.print(post.getPostList().getEntry(i) + ":#:");
					out.print(post.getTimeStampList().getEntry(i) + ":#:");
				}
				out.print("/$%^&/" + ":#:" + "0000/00/00 00:00:00" + ":#:" + "\n"); // end of line (for 1 account)
			}
