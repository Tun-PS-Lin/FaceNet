...
public class Main_Program {
	...
	public void mainProgram() throws IOException {
		...
		boolean isFileExist;
		Scanner inputMaster, inputPost;

		File infileMaster = new File("masterList.txt");
		File infilePost = new File("postList.txt");
		
		inputMaster = new Scanner(infileMaster);
		inputPost = new Scanner(infilePost);


		populateProfiles(inputMaster);
		populatePost(inputPost);
		// d.displayProfileTest(ID_profile); //WORK

		/**
		 * populate ID_post dictionary
		 */

		inputMaster = new Scanner(System.in);
	 ...
	 * @throws IOException 
	 */
	//public void profileMainMenu(Scanner inputMaster, Scanner inputPost, String ID, Profile profile) {
	public void profileMainMenu(Scanner inputMaster, String ID, Profile profile) throws IOException {
		String confirmRemove;
		int choice = profileMainDisplay(inputMaster);
		inputMaster.nextLine();
		...
		while (choice != 6) {
			if (choice == 1) {
				...
				updateProfile(inputMaster, ID, profile);
			} else if (choice == 4) {
				viewFriends(inputMaster, ID, profile);
			} else {
				System.out.println("\nDo you want to delete your profile? (Y/N)");
				System.out.print("Enter choice: ");
				confirmRemove = inputMaster.nextLine();
			...
			choice = profileMainDisplay(inputMaster);
			inputMaster.nextLine();
		}
		System.out.println("\nSuccessful Log-out!\n");
	}
	 ...
	 * @return
	 */
	public String viewPostMenuDisplay(Scanner input) {
		String choice;
		System.out.println("\n-----------------------------------------------------------------------------------");
	 ...
	 * @param ID
	 */
	public void viewPost(Scanner input, String ID) {
		String choice = viewPostMenuDisplay(input);
		while (!choice.equals("X")) {
			if (choice.equals("A")) {
				displayFriendPost(input, ID);
			} else if (choice.equals("B")) {
				uploadPost(input, ID);
			...
			System.out.println("\n");
			choice = viewPostMenuDisplay(input);
		}
	}
	
	/**
	 * Display friends post within 24 hours
	 */
	public void displayFriendPost(Scanner input, String ID) {
	 ...
	 */
	public void uploadPost(Scanner input, String ID) {
		String userPost;
		AList<String> postList;
		
		System.out.println("What's new today? Share with your friends: \n");
		userPost = input.nextLine();
		
		postList = ID_post.getValue(ID).getPostList();
		postList.add(userPost);
		ID_post.getValue(ID).setPostList(postList);
	}

	/**
	 * view the user own post that is within 24 hours at a time
	 */
	public void viewMyPost(String ID) {
		System.out.println("\nMY POSTS WITHIN 24 HOURS:\n");
	 ...
	 * @param input
	 */
	public void populatePost(Scanner input) {
		{
			String ID = null, userPost = null; 
			// String timeStamp = null, like = null, comment null;
			Post post;
			...
			while (input.hasNext()) {
				...
				post = new Post(postList);
				// Add post and ID to the dictionary database
				ID_post.add(ID, post);
			}
			input.nextLine();
		}
	...
	public void writePostToFile() throws IOException {
		...
		String ID;
		//String userPost, timeStamp;
		Post post;

		try {
			FileWriter writeProfile = new FileWriter("postList.txt");
			...
			while (keyIterator.hasNext() && valueIterator.hasNext()) {
				...
				out.print(ID + ":");
				post.getPostList().writePost(out);
